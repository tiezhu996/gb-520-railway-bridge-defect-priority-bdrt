...
type InspectionRoundHandler struct {
	service service.InspectionRoundService
}

func NewInspectionRoundHandler(s service.InspectionRoundService) *InspectionRoundHandler {
	return &InspectionRoundHandler{service: s}
}
...
func (h *InspectionRoundHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *InspectionRoundHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *InspectionRoundHandler) create(c *gin.Context) {
	var input dto.CreateInspectionRound
...
func (h *InspectionRoundHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateInspectionRound
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *InspectionRoundHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *InspectionRoundHandler) remove(c *gin.Context) {
